...
def sendMessage():
    ...
    if(msgtosend=="y" or msgtosend=="Y"):
        ...
        file.close()
        ftp_server.quit()
        print("file successfully downloaded to the path"+download_path)
        textarea.see("end")
...
def openChatWindow():
    ...
    namelabel = Label(window, text= "Enter Your Name", font = ("Arial",10))
    ...
    connectserver = Button(window,text="Connect to Chat Server",bd=1, font = ("Arial",10), command = connectToServer)
    connectserver.place(x=350,y=6)

    separator = ttk.Separator(window, orient='horizontal')
    separator.place(x=0, y=35, relwidth=1, height=0.1)

    labelusers = Label(window, text= "Active Users",fg="blue", font = ("Arial",10))
    labelusers.place(x=10, y=50)

    listbox = Listbox(window,height = 5,width = 67,activestyle = 'dotbox', font = ("Arial",10))
    ...
    connectButton=Button(window,text="Connect",fg="green",bd=1, font = ("Arial",10), command = connectWithClient)
    connectButton.place(x=282,y=160)

    disconnectButton=Button(window,text="Disconnect",fg="red",bd=1, font = ("Arial",10), command = disconnectWithClient)
    disconnectButton.place(x=350,y=160)

    refresh=Button(window,text="Refresh",fg="blue",bd=1, font = ("Arial",10), command = showClientsList)
    refresh.place(x=435,y=160)

    labelchat = Label(window, text= "Chat Window",fg="red", font = ("Arial",10))
    labelchat.place(x=10, y=180)

    textarea = Text(window, width = 67,height = 6,font = ("Arial",10))
    ...
    attach=Button(window,text="Attach & Send",fg="blue",bd=1, font = ("Arial",10), command = browseFiles)
    attach.place(x=10,y=305)

    text_message = Entry(window, width =43, font = ("Arial",12))
    text_message.pack()
    text_message.place(x=98,y=306)

    send=Button(window,text="Send",fg="blue",bd=1, font = ("Arial",10), command = sendMessage)
    send.place(x=450,y=305)

    filePathLabel = Label(window, text= "",fg= "green", font = ("Arial",8))
